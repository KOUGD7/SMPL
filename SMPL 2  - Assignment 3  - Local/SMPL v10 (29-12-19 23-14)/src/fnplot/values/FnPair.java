...
public class FnPair extends FnPlotValue<FnPlotFunction> {
   ...
   public void setCar(FnPlotValue f) {
	...
	else if (! Fn.getCar().equals(car)) {
		return false;
	}
	else if (obj instanceof FnPair) {
		if ((Fn.getCar() == null && car == null) || (Fn.getCar().equals(car) && ((Fn.getCdr() == null && cdr == null) || Fn.getCdr().equals(cdr)))){ 
			return true;
		}
	}
	return false;
    ...
    public String toString() {
        ...
        return String.format("[R%s(%s, %s)]", size, car.toString(), cdr.toString());
    }
